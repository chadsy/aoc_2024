...
//
// Driver/CLI for the Advent of Code 2024 contest
...
// Step 1: create a new dayXX.c from template.c and add it to the
//         CMakeLists.txt makefile, in the `set(SOURCE_FILES...` list
...
// Step 2 here, external decls for the solver functions
...
// Step 3 here, data structures to hold the solver list
...
// Step 4 here
dataset datasets[] = {
        {day01, true, "inputs/sample01.txt", 11, 31},
        {day01, false, "inputs/day01.txt", 3569916, 26407426},
    ...
    for (int i = 0; i < solver_count; i++) {
        ...
        for (int j = 0; j < countof(datasets); j++) {
            ...
            if (datasets[j].day == se.day) {
                ...
                if (!runinfo.input) {
                    int err = errno;
                    fprintf(stderr, "Error %d opening %s: %s\n", err, data.input.filename, strerror(err));
                    exit(err);
                }
...
void show_help(void) {
    printf("aoc - CLI driver for AOC 2024 puzzles\n");
